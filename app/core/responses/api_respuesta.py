...
class ApiDeRespuesta:
    ...
    def exito(mensaje_enum, datos=None, codigo=200):
        ...
        respuesta = {'estado': codigo, 'mensaje': mensaje, 'datos': datos}
        return JSONResponse(status_code=codigo, content=jsonable_encoder({k: v for k, v in respuesta.items() if v is not None}))
    ...
    def error(mensaje_enum, detalles=None, codigo=None, errores=None, request: Optional[Request]=None):
        ...
        respuesta = {'estado': codigo_final, 'mensaje': mensaje, 'detalles': detalles, 'ruta': request.url.path if request else None, 'fecha': ahora.strftime('%Y-%m-%d'), 'hora': ahora.strftime('%H:%M:%S'), 'errores': errores}
        return JSONResponse(status_code=codigo_final, content=jsonable_encoder({k: v for k, v in respuesta.items() if v is not None}))
